    ...
    where TModifier : struct, Enum
{
    ...
    private readonly List<InstrumentSpecialPhrase> bigRockEndings = [];

    protected virtual byte BigRockCount => 0;

    public virtual ILaneInstrumentReadMapper Mapper { get; }

    public LaneInstrumentParser(ILaneInstrumentReadMapper mapper, MidiReadingSession session) : base(session)
    {
        Mapper = mapper;
        ...
        switch (mapping.State)
        {
            case NoteState.Open:
                if (track is null)
                {
                    ...
                    {
                        if (Session.HandleUnclosed(openedPosition.Value))
                        {
                            InitTracks();
                ...
                {
                    if (openedPosition is not null && Session.HandleUnclosed(openedPosition.Value))
                        CloseSpecial(track);
            ...
            case NoteState.Close:
                if (track is null)
                {
                    ...
                    {
                        if (Session.HandleUnopened(mapping.Position))
                        {
                            InitTracks();

                            foreach (var t in tracks)
                                t.SpecialPhrases.Add(new(mapping.Position, type));
    ...
    protected virtual void HandleNote(NoteEventMapping mapping)
    {
        ...
        switch (mapping.State)
        {
            case NoteState.Open:
                ...
                {
                    if (Session.HandleUnclosed(openedSource.Position))
                ...
                Session.HandleDuplicate(chord.Position, "note", () => chord.Notes.Contains(lane));
            ...
            case NoteState.Close:
                ...
                {
                    if (Session.HandleUnopened(mapping.Position))
                        GetOrCreateChord(mapping.Position, track).Notes.Add(lane);
                }
                else
                {
                    openedSource.Notes[lane]!.Sustain = GetSustain(openedSource.Position, mapping.Position);
    ...
    protected virtual void HandleBigRock(NoteEventMapping mapping)
    {
        ...
        switch (mapping.State)
        {
            ...
            case NoteState.Close:
                ...
                {
                    if (Session.HandleUnopened(mapping.Position))
                        GetInstrument().SpecialPhrases.Add(new(mapping.Position, InstrumentSpecialPhraseType.BigRockEnding));
                }
                else
                {
    ...
    private TChord GetOrCreateChord(uint newChordPosition, Track<TChord> track)
    {
        ...
        if (chord is null)
            ...
            return Session.HandleSnap(chord.Position, newChordPosition) ? chord : Create();
        else
            return Create();
