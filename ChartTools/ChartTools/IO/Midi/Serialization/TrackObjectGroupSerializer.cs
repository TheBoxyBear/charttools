	...
	: GroupSerializer<T, MidiEvent, IMidiEventMapping>(header, content)
{
	...
	protected override IEnumerable<MidiEvent> CombineProviderResults(IEnumerable<IMidiEventMapping>[] mappings)
		=> mappings.AlternateBy(mapping => mapping.Position).RelativeLoop().Select(pair =>
		{
