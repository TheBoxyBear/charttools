...
public static class MidiFile
{
	...
	{
		byte count = 0;
		MidiReadingSession session = new(config, formatting ?? new());
		MidiFileReader reader      = new(path, header => GetSongParser(header, session, ref count), config?.FirstPassReadingSettings);
