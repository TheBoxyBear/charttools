...
internal class StandardInstrumentParser : LaneInstrumentParser<StandardChord, StandardLane, StandardChordModifiers>
{
	...
	public StandardInstrumentParser(StandardInstrumentMapper mapper, StandardInstrumentIdentity instrument, MidiReadingSession session) : base(mapper, session)
	{
	...
	protected override void AddModifier(StandardChord chord, byte index)
		=> chord.Modifiers |= (StandardChordModifiers)index;
	...
	protected override void FinaliseParse()
	{
		...
		if (origin == MidiInstrumentOrigin.Unknown)
			origin = Session.HandleUncertainFormat(Instrument, origin);
