...
internal abstract class InstrumentMapper<TChord> : IReadMapper, IInstrumentWriteMapper<TChord>
	where TChord : Chord, new()
{
	...
	protected InstrumentMapper(MidiReadingSession session)
		: this(session, new(MidiFile.DefaultWriteConfig, new())) { }

	protected InstrumentMapper(MidiWritingSession session)
		: this(new(MidiFile.DefaultReadConfig, new()), session) { }
	...
	protected void HandleInvalidMidiEvent(uint position, NoteEvent e)
		=> ReadingSession.HandleInvalidMidiEvent(position, e);
