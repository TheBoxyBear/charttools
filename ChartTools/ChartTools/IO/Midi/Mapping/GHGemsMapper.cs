...
internal class GHGemsMapper : StandardInstrumentMapper, IAnimationContainer<VocalistMouthEvent>
{
	...
	public GHGemsMapper(MidiReadingSession session)
		: base(session) { }

	public GHGemsMapper(MidiWritingSession session, IEnumerable<VocalistMouthEvent> vocalistAnimations)
		: base(session) => animations = [.. vocalistAnimations];
	...
	public override IEnumerable<NoteMapping> Map(Instrument<StandardChord> instrument)
	{
		foreach (var track in instrument.GetExistingTracks())
		{
			...
			foreach (var chord in track.Chords.Where(c => c.Modifiers == StandardChordModifiers.None || ICommonWritingConfiguration.GetUnsupportedModifierChordFlags(c.Position, WritingSession.Configuration.UnsupportedModifiersPolicy).HasFlag(ChordDataFlags.Chord)))
